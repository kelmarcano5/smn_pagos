package domain.repgen.smn_proceso.smn_anticipo;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("smn_entidad_rf_name");
			if (d != null)
			{
			name  = "b_smn_entidad_rf";
			label = "${lbl:b_smn_entidad_rf}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_documento_id_name");
			if (d != null)
			{
			name  = "b_smn_documento_id";
			label = "${lbl:b_smn_documento_id}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("ant_fecha_registro_init");
			if (d != null)
			{
		...
		d = rs.getString("ant_numero_documento");
			if (d != null)
			{
		...
		d = rs.getString("smn_proveedor_rf_name");
			if (d != null)
			{
			name  = "b_smn_proveedor_rf";
			label = "${lbl:b_smn_proveedor_rf}";
		...
		d = rs.getString("ant_fecha_registro_end");
			if (d != null)
			{
			name  = "b_store_date}@{\${def:date}";
			label = "${lbl:b_store_date}@{\${def:date}}";
