...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("doc_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("smn_documentos_generales_rf");
			if (d != null)
			{
			name  = "b_general_document";
			label = "${lbl:b_general_document}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_modulo_origen_rf_name");
			if (d != null)
			{
		...
		d = rs.getString("doc_codigo");
			if (d != null)
			{
			name  = "b_code_document";
			label = "${lbl:b_code_document}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_tipo_documento_id");
			if (d != null)
			{
			name  = "b_document_type_id";
			label = "${lbl:b_document_type_id}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("doc_descripcion");
			if (d != null)
			{
			name  = "b_description_document";
			label = "${lbl:b_description_document}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_modulo_origen_rf");
			if (d != null)
			{
			name  = "b_source_module";
			label = "${lbl:b_source_module}";
		...
		d = rs.getString("smn_documentos_generales_rf_name");
			if (d != null)
			{
			name  = "b_general_document";
			label = "${lbl:b_general_document}";
