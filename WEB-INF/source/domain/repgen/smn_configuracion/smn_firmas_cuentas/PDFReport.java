package domain.repgen.smn_configuracion.smn_firmas_cuentas;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("fcb_cantidad");
			if (d != null)
			{
			name  = "b_quantity_fcb";
			label = "${lbl:b_quantity_fcb}";
		...
		d = rs.getString("fcb_descripcion");
			if (d != null)
			{
			name  = "b_description_fcb";
			label = "${lbl:b_description_fcb}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_tipo_firma_id_name");
			if (d != null)
			{
			name  = "b_type_firm_fcb";
			label = "${lbl:b_type_firm_fcb}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_tipo_firma_id");
			if (d != null)
			{
			name  = "b_type_firm_fcb";
			label = "${lbl:b_type_firm_fcb}";
		...
		d = rs.getString("fcb_codigo");
			if (d != null)
			{
			name  = "b_code_fcb";
			label = "${lbl:b_code_fcb}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("fcb_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("smn_cuentas_bancarias_rf_name");
			if (d != null)
			{
			name  = "b_bank_account";
		...
		d = rs.getString("smn_cuentas_bancarias_rf");
			if (d != null)
			{
			name  = "b_bank_account";
			label = "${lbl:b_bank_account}";
