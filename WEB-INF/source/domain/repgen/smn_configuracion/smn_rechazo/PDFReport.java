package domain.repgen.smn_configuracion.smn_rechazo;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("rec_codigo");
			if (d != null)
			{
			name  = "b_code_rec";
			label = "${lbl:b_code_rec}";
		...
		d = rs.getString("rec_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("rec_descripcion");
			if (d != null)
			{
			name  = "b_code_rec";
			label = "${lbl:b_code_rec}";
