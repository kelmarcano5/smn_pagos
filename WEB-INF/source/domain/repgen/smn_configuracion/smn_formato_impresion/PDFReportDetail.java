package domain.repgen.smn_configuracion.smn_formato_impresion;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("foi_texto");
			if (d != null)
			{
			name  = "b_foi_text";
			label = "${lbl:b_foi_text}";
		...
		d = rs.getString("foi_codigo");
			if (d != null)
			{
			name  = "b_foi_code";
			label = "${lbl:b_foi_code}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("foi_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("foi_descripcion");
			if (d != null)
			{
			name  = "b_foi_description";
			label = "${lbl:b_foi_description}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
