package domain.repgen.smn_configuracion.smn_autorizador_firmas_cuenta;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("cdp_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("cdp_descripcion");
			if (d != null)
			{
			name  = "b_description_cdp";
			label = "${lbl:b_description_cdp}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cdp_codigo");
			if (d != null)
			{
			name  = "b_code_cdp";
			label = "${lbl:b_code_cdp}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
