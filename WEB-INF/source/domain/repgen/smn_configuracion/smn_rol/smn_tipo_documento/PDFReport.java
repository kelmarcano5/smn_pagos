package domain.repgen.smn_configuracion.smn_rol.smn_tipo_documento;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("tdo_tipo_movimiento");
			if (d != null)
			{
			name  = "b_movement_type";
			label = "${lbl:b_movement_type}";
		...
		d = rs.getString("tdo_descripcion");
			if (d != null)
			{
			name  = "b_description_tdo";
			label = "${lbl:b_description_tdo}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("tdo_codigo");
			if (d != null)
			{
			name  = "b_code_document_type";
			label = "${lbl:b_code_document_type}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("tdo_fecha_registro");
			if (d != null)
			{
