package domain.repgen.smn_configuracion.smn_tipo_firma;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("tfi_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("tfi_descripcion");
			if (d != null)
			{
			name  = "b_description_tfi";
			label = "${lbl:b_description_tfi}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("tfi_codigo");
			if (d != null)
			{
			name  = "b_code_tfi";
			label = "${lbl:b_code_tfi}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
