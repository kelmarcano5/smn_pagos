package domain.repgen.smn_configuracion.smn_formpag_documento_banco;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("fdb_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("fdb_genera_transaccion");
			if (d != null)
			{
			name  = "b_generate_transaction";
			label = "${lbl:b_generate_transaction}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_forma_pago_rf");
			if (d != null)
			{
			name  = "b_way_to_pay_fdb";
			label = "${lbl:b_way_to_pay_fdb}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
