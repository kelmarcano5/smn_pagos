package domain.repgen.smn_registro.smn_nota_entrada;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_numero_documento_rf");
			if (d != null)
			{
			name  = "b_document_number";
			label = "${lbl:b_document_number}";
		...
		d = rs.getString("nep_monto_saldo_ml");
			if (d != null)
			{
			name  = "b_amount_balance_ml_ne";
			label = "${lbl:b_amount_balance_ml_ne}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("nep_monto_usado_ml");
			if (d != null)
			{
			name  = "b_amount_used_ml_ne";
			label = "${lbl:b_amount_used_ml_ne}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("nep_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("nep_monto_ml");
			if (d != null)
			{
			name  = "b_amoun_ne_ml";
			label = "${lbl:b_amoun_ne_ml}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_proveedor_id_name");
			if (d != null)
			{
			name  = "b_auxiliary_supplier";
			label = "${lbl:b_auxiliary_supplier}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_proveedor_id");
			if (d != null)
			{
			name  = "b_auxiliary_supplier";
			label = "${lbl:b_auxiliary_supplier}";
