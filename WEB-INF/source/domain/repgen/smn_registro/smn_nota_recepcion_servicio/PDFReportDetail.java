package domain.repgen.smn_registro.smn_nota_recepcion_servicio;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("nrs_numero");
			if (d != null)
			{
			name  = "b_number_document";
			label = "${lbl:b_number_document}";
		...
		d = rs.getString("nrs_fecha_registro");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_centro_costo_rf");
			if (d != null)
			{
			name  = "b_cost_center";
			label = "${lbl:b_cost_center}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_orden_compra_rf");
			if (d != null)
			{
			name  = "b_buy_order";
			label = "${lbl:b_buy_order}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_orden_compra_rf_name");
			if (d != null)
			{
			name  = "b_buy_order";
			label = "${lbl:b_buy_order}";
		...
		d = rs.getString("smn_centro_costo_rf_name");
			if (d != null)
			{
			name  = "b_cost_center";
			label = "${lbl:b_cost_center}";
		...
		d = rs.getString("nrs_monto_saldo_ml");
			if (d != null)
			{
			name  = "b_amount_balance_ml_nrs";
			label = "${lbl:b_amount_balance_ml_nrs}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
