package domain.repgen.smn_registro.smn_orden_compra;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("smn_moneda_rf");
			if (d != null)
			{
			name  = "b_currency";
			label = "${lbl:b_currency}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_moneda_rf_name");
			if (d != null)
			{
			name  = "b_currency";
			label = "${lbl:b_currency}";
		...
		d = rs.getString("ocp_monto_saldo_ml");
			if (d != null)
			{
			name  = "b_amount_balance_ml";
			label = "${lbl:b_amount_balance_ml}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("ocp_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("ocp_monto_ml");
			if (d != null)
			{
			name  = "b_amount_ml";
			label = "${lbl:b_amount_ml}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
