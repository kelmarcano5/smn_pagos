package domain.repgen.smn_registro.smn_impuestos_retenciones;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("iop_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("iop_base_imponible");
			if (d != null)
			{
			name  = "b_tax_base";
			label = "${lbl:b_tax_base}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("iop_monto_impuesto");
			if (d != null)
			{
			name  = "b_tax_amount";
			label = "${lbl:b_tax_amount}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
