package domain.repgen.smn_registro.smn_descuento_retencion;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
			...
			if (d != null)
			{
			name  = "b_provider_class";
			label = "${lbl:b_provider_class}";
		...
		d = rs.getString("smn_descuento_retencion_rf");
			if (d != null)
			{
			name  = "b_code_discount";
			label = "${lbl:b_code_discount}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("dro_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("dro_porcentaje");
			if (d != null)
			{
			name  = "b_discount_percentage";
			label = "${lbl:b_discount_percentage}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_descuento_retencion_rf_name");
			if (d != null)
			{
			name  = "b_code_discount";
			label = "${lbl:b_code_discount}";
		...
		d = rs.getString("hidden");
			if (d != null)
			{
			name  = "b_payment_order_id";
			label = "${lbl:b_payment_order_id}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("dro_monto_descuento");
			if (d != null)
			{
			name  = "b_percentage_amount";
			label = "${lbl:b_percentage_amount}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("dro_monto_base");
			if (d != null)
			{
			name  = "b_amount_base";
			label = "${lbl:b_amount_base}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
