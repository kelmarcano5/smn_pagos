package domain.repgen.smn_registro.smn_control_documento;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("dpd_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("dpd_fecha_anulacion");
			if (d != null)
			{
			name  = "b_cancellation_date";
			label = "${lbl:b_cancellation_date}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("dpd_fecha_uso");
			if (d != null)
			{
			name  = "b_date_use";
			label = "${lbl:b_date_use}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("dpd_estatus");
			if (d != null)
			{
			name  = "b_process_status";
			label = "${lbl:b_process_status}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("dpd_numero_documento");
			if (d != null)
			{
