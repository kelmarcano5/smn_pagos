package domain.repgen.smn_registro.smn_plan_descuento_retencion;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("pdr_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("pdr_fecha_hasta");
			if (d != null)
			{
			name  = "b_date_up";
			label = "${lbl:b_date_up}";
		...
		d = rs.getString("smn_descuento_retencion_rf");
			if (d != null)
			{
			name  = "b_retention_discount";
			label = "${lbl:b_retention_discount}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("pdr_fecha_desde");
			if (d != null)
			{
			name  = "b_date_from";
			label = "${lbl:b_date_from}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
