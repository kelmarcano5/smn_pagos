SELECT
	...
	proveedor.aux_descripcion AS nombre_pvdor,
	...
	CASE 
	 when (opa_fecha_vencimiento) < to_date(${fld:opa_fecha_plan}, 'DD-MM-YYYY')
	  then
			opa_saldo_factura_ml
	END AS total_pvd
FROM
	...
	LEFT JOIN smn_compras.smn_proveedor ON  smn_compras.smn_proveedor.smn_auxiliar_rf = smn_pagos.smn_orden_pago.smn_auxiliar_rf
	INNER JOIN smn_base.smn_auxiliar proveedor ON proveedor.smn_auxiliar_id = smn_pagos.smn_orden_pago.smn_auxiliar_rf
